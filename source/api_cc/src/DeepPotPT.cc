...
void DeepPotPT::compute(ENERGYVTYPE& ener,
  ...
  torch::Device device(torch::kCUDA, gpu_id);
  ntypes = *max_element(atype.begin(), atype.end()) + 1;
  ...
  // select real atoms
  std::vector<VALUETYPE> dcoord, dforce, dforce_mag, aparam_, datom_energy, datom_virial;
  ...
  if (nloc == 0) {
    ...
    fill(force_mag.begin(), force_mag.end(), (VALUETYPE)0.0);
    // dvirial of size 9
    virial.resize(static_cast<size_t>(nframes) * 9);
    fill(virial.begin(), virial.end(), (VALUETYPE)0.0);
    // datom_energy_ of siznlist_datae nall
  ...
  // std::cout << "neighbor: " << firstneigh_tensor << std::endl;
  ...
  // std::cout << "atype: " << atype_Tensor << std::endl;
  // std::cout << "spin: " << spin_wrapped_Tensor << std::endl;
  
  // std::cout << "Error will occur...." << std::endl;
  c10::Dict<c10::IValue, c10::IValue> outputs =
      module
          .run_method("forward_lower", coord_wrapped_Tensor, atype_Tensor, spin_wrapped_Tensor,
  ...
  // std::cout << "555555555222222222occur...." << std::endl;
  ...
  torch::Tensor flat_force_mag_ = force_mag_.toTensor().view({-1}).to(floatType);
  torch::Tensor cpu_force_mag_ = flat_force_mag_.to(torch::kCPU);
  dforce_mag.assign(cpu_force_mag_.data_ptr<VALUETYPE>(),
  ...
  */
  ...
  select_map<VALUETYPE>(force, dforce, bkw_map, 3, nframes, fwd_map.size(),
                        nall_real);
  select_map<VALUETYPE>(force_mag, dforce_mag, bkw_map, 3, nframes, fwd_map.size(),
                        nall_real);
...
void DeepPotPT::compute(ENERGYVTYPE& ener,
  ...
  if (natoms == 0) {
    ...
    fill(force_mag.begin(), force.end(), (VALUETYPE)0.0);
    // dvirial of size 9
    virial.resize(static_cast<size_t>(nframes) * 9);
    fill(virial.begin(), virial.end(), (VALUETYPE)0.0);
  ...
  at::Tensor spin_wrapped_Tensor =
      torch::from_blob(spin_wrapped.data(), {1, natoms, 3}, options)
          .to(device);
  ...
  torch::Tensor flat_force_mag_ = force_mag_.toTensor().view({-1}).to(floatType);
  torch::Tensor cpu_force_mag_ = flat_force_mag_.to(torch::kCPU);
  force_mag.assign(cpu_force_mag_.data_ptr<VALUETYPE>(),
               cpu_force_mag_.data_ptr<VALUETYPE>() + cpu_force_mag_.numel());
  // torch::Tensor flat_virial_ = virial_.toTensor().view({-1}).to(floatType);
  // torch::Tensor cpu_virial_ = flat_virial_.to(torch::kCPU);
  // virial.assign(cpu_virial_.data_ptr<VALUETYPE>(),
