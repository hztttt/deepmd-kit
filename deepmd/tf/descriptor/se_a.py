...
class DescrptSeA(DescrptSe):
    ...
    def build(
        ...
        if self.spin is not None:
            ...
            rj_gathered = tf.gather(diff_coord_reshape, tf.maximum(nlist_reshaped, 0), axis=1, batch_dims=1)
    ...
    def natoms_match(self, coord, natoms):
        ...
        for i in range(self.ntypes):
            if i + self.ntypes_spin >= self.ntypes:
                ...
                diff_coord_loc.append(tf.zeros([tf.shape(coord)[0], natoms[2 + i] * 3], dtype=coord.dtype))
    ...
    def natoms_not_match(self, coord, natoms, atype):
        ...
        for i in range(self.ntypes):
            if i + self.ntypes_spin >= self.ntypes:
                ...
                diff_coord_ghost.append(
                    tf.zeros([tf.shape(coord)[0], ghost_natoms[i] * 3], dtype=coord.dtype)                
                )
